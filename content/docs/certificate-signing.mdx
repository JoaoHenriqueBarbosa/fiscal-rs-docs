...
sequenceDiagram
    ...
    Sign->>RSA: parse private_key via RsaPrivateKey::from_pkcs8_pem
    RSA->>RSA: Pkcs1v15Sign::&lt;Sha1&gt;::sign(...)
...
A chave privada PEM é parseada com o crate `rsa` (RustCrypto), e o `<SignedInfo>` canônico é assinado com RSA-SHA1 via PKCS#1 v1.5:

```rust
let rsa_private = rsa::RsaPrivateKey::from_pkcs8_pem(&private_key_pem)?;
let signature_bytes = rsa_private.sign(
...
A assinatura é determinística (PKCS#1 v1.5), produzindo saída byte-idêntica ao OpenSSL, garantindo compatibilidade retroativa com verificadores existentes.
...
## Implementação interna (RustCrypto)

O `fiscal-crypto` agora é 100% Rust, sem dependência de OpenSSL. A migração substituiu o parser ASN.1 manual e a assinatura via `openssl-sys` por crates do ecossistema RustCrypto:

| Funcionalidade | Antes (OpenSSL) | Agora (RustCrypto) |
|---|---|---|
| Parsing PFX/PKCS#12 | Parser ASN.1 manual (~590 linhas) | `pkcs12` + `pkcs5` |
| KDF PBES1 | Implementação própria | `pkcs12::kdf::derive_key_utf8::<sha1::Sha1>` |
| Assinatura RSA-SHA1 | `PKey` + `Signer` + `MessageDigest` | `rsa::RsaPrivateKey` + `Pkcs1v15Sign` |
| HMAC (MAC) | Não verificado | HMAC SHA-256 / SHA-1 antes da decriptação |
| BER indefinite-length | Parser manual com `len_byte == 0x80` | Delegado ao `pkcs12` crate |

### Verificação MAC

Antes de decriptar o PFX, o `fiscal-crypto` agora **recalcula o HMAC** e compara com o valor armazenado. Isso detecta senha incorreta antes de qualquer tentativa de decriptação:

```rust
// Internamente (simplificado):
let pfx = Pkcs12::from_der(pfx_bytes)?;
pfx.parse2(passphrase)?; // valida MAC + extrai chaves
// Se a senha estiver errada, retorna erro imediatamente
// sem tentar decriptar com chave errada
```

Isso garante erro explícito `"Failed to parse PFX (wrong password?)"` em vez de panic de slice indexing (como ocorria com 3DES no parser antigo).

### BER indefinite-length

O formato PFX usa BER (Basic Encoding Rules) que suporta comprimento indefinido (`0x80`). O parser manual antigo tratava `len_byte == 0x80` como comprimento zero, resultando em parse silenciosamente incorreto. Agora o `pkcs12` crate gerencia BER corretamente, rejeitando PFX malformados com erro descritivo.

### `ensure_modern_pfx`

A função `ensure_modern_pfx` foi de no-op (não fazia nada) para validação real que verifica se o PFX usa um esquema de criptografia moderno (PKCS#12 v1.1+). Recomenda-se chamá-la após `load_certificate` para certificados que precisam ser reexportados para uso em outras ferramentas.

## Assinatura automática de eventos no SefazClient

O `SefazClient::new` agora carrega o certificado internamente via `fiscal_crypto::load_certificate` e **assina automaticamente** os eventos XML antes de transmiti-los para a SEFAZ:

- Cancelamento (`cancel`)
- Carta de Correção (`cce`)
- Manifestação do Destinatário (`manifest`)
- EPEC, prorrogação, conciliação, entrega, RTC

Cada `<evento>` individual em lotes também é assinado separadamente. Antes da PR #51, os eventos eram enviados sem assinatura no `<infEvento>`, causando rejeição em produção.
...
// Eventos sao assinados automaticamente antes do envio
let response = client.cancel(uf, env, &access_key, &protocol, "justificativa", &cnpj).await?;
```
