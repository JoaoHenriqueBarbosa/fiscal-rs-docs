...
stateDiagram-v2
    ...
    Draft --> Draft : .series() / .add_item() / .recipient() / ...
    Draft --> Built : .build()
    Built --> Signed : .sign_with(closure)
    Signed --> [*] : .signed_xml()

    state Draft {
        [*] --> Configuração
        Configuração : Campos do emissor, itens,
        Configuração : pagamentos, transporte, etc.
    }

    state Built {
        [*] --> XMLGerado
        XMLGerado : Chave de acesso calculada
        XMLGerado : XML unsigned disponível
    }

    state Signed {
        [*] --> XMLAssinado
        XMLAssinado : XML-DSig RSA-SHA1 C14N
        XMLAssinado : Pronto para transmissão
    }
`} />
