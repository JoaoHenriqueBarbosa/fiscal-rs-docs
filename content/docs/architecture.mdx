...
| `fiscal-crypto` | Carregamento de certificado digital (PFX) e assinatura XML-DSig | `pkcs12`, `rsa`, `sha1`, `base64`, `chrono` |
...
RustCrypto (pkcs12, rsa, sha1), carregamento PFX, assinatura XML-DSig"]
    end
...
- **`fiscal-crypto`**: usa RustCrypto (crates `pkcs12`, `rsa`, `sha1`) para extrair chaves de certificados `.pfx` e assinar XML.
- **`fiscal-sefaz`**: faz requisições HTTP async via `reqwest` com mTLS para os webservices da SEFAZ.

Essa separação permite que o core seja compilado e testado sem dependências de criptografia e sem acesso à rede.
...
Cada crate pode ser publicado separadamente no [crates.io](https://crates.io). Um projeto que só precisa gerar XML fiscal pode depender de `fiscal-core` sem arrastar dependências de criptografia ou `reqwest` como dependências transitivas.
...
O `fiscal-core` não tem dependências de criptografia nem de I/O de rede, tornando-o ideal para:
